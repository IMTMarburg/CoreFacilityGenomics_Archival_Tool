...
import { add_task, update_task, load_tasks, load_workingdir_runs } from "$lib/util";

async function pending_deletions() {
  let open_deletions = [];
  let tasks = await load_tasks();
  for (let task of tasks) {
    if (task["type"] == "delete_run" && (task["status"] == "open" || task["status"] == "processing")) {
...
export const actions = {
  abort: async ({ cookies, request }) => {
    ...
    try {
	  ...
	  for (let open_deletion of data.open_deletions) {
		  ...
		  if (open_deletion["run"] == form_data.get("run") && open_deletion.status == "open") {
			  update_task(open_deletion, {status: "aborted", 
  ...
  delete: async ({ cookies, request, locals }) => {
    ...
    try {
      ...
      if (!found) {
        throw new Error("Run not found (or not eligble for deletion)");
      }
      add_task("delete_run", {
        run: form_data.get("run"),
