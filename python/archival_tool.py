...
"""
    tf = tempfile.NamedTemporaryFile(suffix=".sh", mode="w", delete=False)
    tf.write(script)
    tf.flush()
    result = subprocess.run(
        ["bash", str(tf.name), task["batch_no"], task["password"]],
        capture_output=True,
        text=True,
    )
    ...
    if result.returncode != 0:
        ...
        update_task(
            task, {"status": "failed", "finish_time": int(time.time()), **output}
        )
        add_event(
            {"type": "novogene_download_failed", "batch_no": task["batch_no"], **output}
        )
    else:
...
def make_sure_we_have_bash():
    from shutil import which

    assert which("bash"), "no bash present"
